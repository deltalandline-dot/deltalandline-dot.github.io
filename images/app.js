import {configured,listPhotos,publishPhoto,preparedPhoto,accessToken,signIn,acceptSignInLink,signOut,listCollection,setPhotoFeatured,removePhoto} from '../shared/connection.js?v=f8ce8c71522b';
import {DISPLAY_MS,choosePhoto,frameSize} from './gallery.js?v=f8ce8c71522b';
...
async function show(index){if(!photos.length)return;const token=++generation,item=photos[index],loaded=new Image();loaded.src=item.url;try{await loaded.decode();}catch{if(token!==generation)return;if(token===generation){$('empty').textContent='This photograph couldn’t be loaded.';$('empty').hidden=false;}schedule();return;}if(token!==generation)return;current=index;showing=item;$('photo').src=loaded.src;$('photo').alt=item.description||'Photograph';resize();$('frame').hidden=false;$('empty').hidden=true;schedule();}
async function loadGallery(){const request=++galleryRequest;try{const next=await listPhotos();if(request!==galleryRequest)return;photos=next;if(photos.length){const retained=photos.findIndex(p=>p.id===showing?.id);await show(retained>=0?retained:choosePhoto(photos,null));}else{$('frame').hidden=true;$('empty').hidden=false;$('empty').textContent='No photographs yet.';schedule();}}catch{if(request===galleryRequest){$('empty').textContent='The gallery couldn’t be loaded. Please try again.';schedule();}}}
